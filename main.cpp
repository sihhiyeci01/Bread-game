...
LRESULT CALLBACK LowLevelMouseProc(int nCode, WPARAM wParam, LPARAM lParam) {
	...
	else if (nCode == HC_ACTION && wParam == WM_LBUTTONUP) {
        isEventMouse.store(true);
...
void sleam(){
    ...
    if (!mouseHook) {
        std::cerr << "Mouse hook kurulamadi!" << std::endl;
        exit(0);
    }

    // Mesaj döngüsü
    MSG msg;
    while (GetMessage(&msg, nullptr, 0, 0) > 0 && g_running.load()) {
...
void setConsoleSize(int width, int height) {
    ...
    // Mevcut buffer boyutlarını öğren
    CONSOLE_SCREEN_BUFFER_INFO csbi;
    if (!GetConsoleScreenBufferInfo(hConsole, &csbi)) return;

    SHORT curBufW = csbi.dwSize.X;
    SHORT curBufH = csbi.dwSize.Y;

    // Hedef window bölgesi
    SMALL_RECT windowSize = { 0, 0, static_cast<SHORT>(width - 1), static_cast<SHORT>(height - 1) };
    // Hedef buffer boyutu (min genislik = current veya requested)
    COORD bufferSize = {
        static_cast<SHORT>(std::max<SHORT>(width,  csbi.dwSize.X)),
        static_cast<SHORT>(std::max<SHORT>(height, csbi.dwSize.Y))
    };

    if (width <= curBufW && height <= curBufH) {
        // **Küçültüyorsan** önce pencereyi, sonra tamponu
        SetConsoleWindowInfo(hConsole, TRUE, &windowSize);
        SetConsoleScreenBufferSize(hConsole, { (SHORT)width, (SHORT)height });
    } else {
        // **Büyütüyorsan** önce tamponu, sonra pencereyi
        SetConsoleScreenBufferSize(hConsole, bufferSize);
        SetConsoleWindowInfo(hConsole, TRUE, &windowSize);
    }

    // Opsiyonel: ekranı temizleyip imleci başa al
    COORD home = { 0, 0 };
    DWORD written;
    FillConsoleOutputCharacter(hConsole, ' ', width * height, home, &written);
    SetConsoleCursorPosition(hConsole, home);
}
void sendTwilioCall() {
    const char* endpoint = "https://twilio-alarm.onrender.com/hello/";
    if (!endpoint) {
        std::cerr << "TWILIO_NODE_CALL_URL ortam değişkeni eksik!" << std::endl;
        return;
    }

    std::string computerName = getComputerName();
    std::string jsonBody = R"({"pcName":")" + computerName + R"("})";
...
void setCustomConsoleColorPalette() {
	...
	csbiex.ColorTable[4] = RGB(210, 210, 210);
	
	csbiex.ColorTable[6] = RGB(188, 188, 188);
	
	csbiex.ColorTable[7] = RGB(140, 140, 140);
	
	csbiex.ColorTable[8] = RGB(80, 80, 80);
...
void printFileCentered(const string& filename) {
    ifstream file(filename);
    if (!file) {
        cerr << "Dosya açilamadi: " << filename << endl;
        return;
    }

    string line;
    while (getline(file, line)) {
        centerText(line, false); // sadece yatay ortala, dikeyde aşağı kay
		cout << endl;
    }
...
void showLicense(){
	...
	printFileCentered("LICENSE.txt");
	
	cout << "\n \n \n \n";
	
	setTextColor(6);
	
	//centerText("Press ENTER to accept the license and continue, or ESC to reject and exit.", false);
	cout << "                   Press ";
	setTextColor(15);
	cout << "ENTER ";
	setTextColor(6);
	cout << "to accept the license and continue, or ";
	setTextColor(15);
	cout << "ESC ";
	setTextColor(6);
	cout << "to reject and exit.";
	char input;
    while (true) 
	{
        input = _getch();
...
int main() {
	...
	if(firstStart==true) showLicense();
